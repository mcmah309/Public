...
public class MeshGeneratorLeft : MonoBehaviour
{
    ...
    void Start()
    {
        transform.position = new Vector3(-(((float) xSize) +7.5f), 0f, -3f);
        mesh = new Mesh();
    ...
    void GenerateNextPart()
    {
        ...
        Vector3[] VertTemp = new Vector3[(xSize + 1) * (zSize + 1)];
        Array.Copy(vertices, xSize +1, VertTemp, 0, ((zSize + 1) * (xSize + 1)) - xSize - 1);

        Globalz += 1;
        for (int x = 0; x <= xSize-1; x++)
        {
            float y = Mathf.PerlinNoise(x * .3f, Globalz * .3f);
            VertTemp[w - 1 - xSize + x] = new Vector3(x, y, Globalz);
        }
        VertTemp[w - 1] = new Vector3(xSize, 0.5f, Globalz);
        ...
        /* Globalz += 1;
         ...
         {
             float y = Mathf.PerlinNoise(x * .3f, Globalz * .3f);
             vertices[w - d + x] = new Vector3(x, y, Globalz);
         }
         vertices[w - 1] = new Vector3(xSize, 0.5f, Globalz);*/
        ...
        verticesList.Add(new Vector3(xSize, 0.5f, Globalz));
        ...
        for (int j = xSize + 1, i = 0; i < d; j++, i++)
        {
            ...
            //vertices[i] = vertices[j] - Vector3.back;
        }
        //Debug.Log(w);
        //Debug.Log(d);
