...
public class RightMeshArray : MonoBehaviour
{
    ...
    void Start()
    {
        transform.position = new Vector3(7.5f, 0f, -20f);
        mesh = new Mesh();
    ...
    void GenerateNextPart()
    {
        ...
        Globalz += 1;
        vertices[u] = new Vector3(0, 0.5f, Globalz);
        ...
        {
            vertices[i] = new Vector3(vertices[j].x, vertices[j].y, vertices[j].z - 1);
        }
        //Debug.Log(w);
        //Debug.Log(d);
        ...
        for (int i = d + 1, x = 1; x < xSize; i++, x++)
        {
            ...
            float y = Mathf.PerlinNoise(x * .3f, Globalz * .3f);
            vertices[i] = (new Vector3(x, y, zSize + 1));
        }
        vertices[d + xSize] = new Vector3(xSize, 0.5f, zSize + 1);
        UpdateMesh();
    }
}
