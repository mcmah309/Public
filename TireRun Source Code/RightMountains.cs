...
public class RightMountains : MonoBehaviour
{
    ...
    void Start()
    {
        transform.position = new Vector3(2f+7.5f, -0.5f, -3f);
        mesh = new Mesh();
    ...
    void CreateShape()
    {
        ...
        for (int i = 0, z = 0; z <= zSize; z++)
        {
            ...
            {
                float y = Mathf.PerlinNoise(x * .3f, z * .3f)*x;
    ...
    void GenerateNextPart()
    {
        ...
        for (int x = 1; x <= xSize-1; x++)
        {
            float y = Mathf.PerlinNoise(x * .3f, Globalz * .3f)*x;
